...
class SelectedIndex(BaseModel):
    ...
    relevance_reasoning: str = Field(
        description="Why this index is relevant to the query"
    )
...
def evaluate_query_results(
    results: List[dict], min_relevant: int = 2, score_threshold: float = 0.6
) -> QueryEvaluation:
...
def off_topic_response(state: State):
    return {
        "final_timeline": "I'm sorry, I can only answer questions related to the medical field."
    }
